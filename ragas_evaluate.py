...
用法：
    python ragas_evaluate.py --dataset ./ragas_output/ragas_dataset.json --modes diagnosis knowledge --batch_size 1 --sleep_per_batch 15.0 --sleep_between_metrics 8.0
...
def main():
    ...
    parser.add_argument("--sleep_per_batch", type=float, default=15.0, help="每批评估完成后 sleep 秒数（默认0）")
    parser.add_argument("--sleep_between_metrics", type=float, default=8.0, help="方案A：每个样本内每个指标之间 sleep 秒数（默认0）。建议 1~10")
